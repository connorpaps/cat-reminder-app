...
- Google Calendar + Google Tasks are read-only with loopback OAuth, encrypted local token storage, calendar/task-list selection, manual refresh, and scheduled sync. Credentials remain release configuration. Google Tasks with a `00:00:00 UTC` due become `all-day` (anchored to local midnight of that UTC date); due-less tasks import as `anytime` (previously dropped). Cloud storage and remote accounts are intentionally out of scope for v1; TickTick integration is planned (research done, `'ticktick'` source reserved).
